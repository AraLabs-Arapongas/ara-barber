...
import { forgotPasswordAction, type ForgotPasswordState } from '@/app/admin/forgot-password/actions'
...
describe('forgotPasswordAction', () => {
  ...
  it('chama resetPasswordForEmail com redirectTo dinâmico (https por default)', async () => {
    ...
    expect(reset).toHaveBeenCalledWith('user@example.com', {
      redirectTo: 'https://qa-aralabs.aralabs.com.br/auth/callback?next=/admin/reset-password',
    })
  ...
  it('usa http em dev quando NODE_ENV !== production', async () => {
    ...
    expect(reset).toHaveBeenCalledWith('user@example.com', {
      redirectTo: 'http://qa-aralabs.lvh.me:3008/auth/callback?next=/admin/reset-password',
    })
