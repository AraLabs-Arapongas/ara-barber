...
export default async function DashboardHome({ searchParams }: PageProps) {
  ...
  // Welcome modal: aparece só se onboarding_step IS NULL (owner ainda
  // não escolheu entre tour e explorar). Após escolha, modal nunca mais.
  const secret = createSecretClient()
  const { data: tenantOnboardingRow } = await secret
    .from('tenants')
    .select('onboarding_step, onboarding_completed_at')
    .eq('id', tenant.id)
    .maybeSingle()
  const showWelcome =
    tenantOnboardingRow?.onboarding_step == null &&
    tenantOnboardingRow?.onboarding_completed_at == null
