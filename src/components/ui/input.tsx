  ...
  return (
    <div className={cn('flex w-full flex-col gap-1.5', containerClassName)}>
      {label ? (
        <label
          htmlFor={inputId}
          className="text-[0.8125rem] font-medium text-fg flex items-center gap-1"
        >
          {label}
          {required ? (
        ...
        className={cn(
          ...
          error
            ...
            : cn(
                ...
                'focus-within:border-brand-primary focus-within:shadow-[0_0_0_3px_color-mix(in_oklch,var(--brand-accent)_25%,transparent)]',
              ),
        )}
      >
        {leftIcon ? (
          <div className="flex shrink-0 items-center pl-3 text-fg-subtle" aria-hidden="true">
            {leftIcon}
          </div>
        ) : null}
        ...
        <input
          ...
          className={cn(
            'min-w-0 flex-1 bg-transparent px-3 py-2.5 text-[0.9375rem] text-fg',
